...
public class DoctorServiceImpl implements DoctorService {
    ...
    // Validation helper for password change
    ...
    public Response changePassword(String username, ChangeDoctorPasswordDTO updateDTO) {
        ...
        try {
            // Attempt to change password via clinic admin service
            return clinicAdminServiceClient.changePassword(username, updateDTO);

        } catch (Exception ex) {
            // Handle any error from Feign or internal failure
            return Response.builder()
                    .success(false)
                    .status(500)
    ...
    public Response login(DoctorLoginDTO loginDTO) {
        ...
        try {
            // Attempt to login via clinic admin service
