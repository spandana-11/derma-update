...
public class PaymentServiceImpl implements PaymentService {
    ...
    public Response createPayment(PaymentDTO paymentDTO) {
        ...
        // Step 1: Get Doctor + ClinicId from ClinicAdmin
        String doctorId;
        String clinicId;
        try {
            ...
            if (e.status() == 400 || (e.status() == 500 && e.getMessage() != null && e.getMessage().toLowerCase().contains("invalid doctor id"))) {
        ...
        try {
            ResponseEntity<ResponseStructure<BookingResponse>> bookingResponseEntity = bookingFeignClient.getBookedService(paymentDTO.getBookingId());
            ...
            if (bookingResponse == null || bookingResponse.getData() == null) {
                return new Response( "Booking not found with ID: " + paymentDTO.getBookingId(), 404, false, null);
            }
            ...
            // Handle Booking Service 500 with invalid booking ID message gracefully
            if (e.status() == 400 || (e.status() == 500 && e.getMessage() != null && e.getMessage().toLowerCase().contains("invalid booking id"))) {
                return new Response("Invalid Booking ID: Please provide a valid booking ID", 400, false, null);
            }
            // Otherwise, generic booking service error
    ...
    public Response getAllPayments() {
        ...
        List<PaymentDTO> dto = new ObjectMapper().convertValue(payments, new TypeReference<List<PaymentDTO>>() {});
        return new Response("Payments fetched successfully",200,true, dto);
    }
