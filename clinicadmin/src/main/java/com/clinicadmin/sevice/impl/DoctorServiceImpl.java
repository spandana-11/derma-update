		...
		try {
			...
			if (doctor != null) {
				...
				DoctorLoginCredentials credentials = DoctorLoginCredentials.builder()
						.doctorId(savedDoctor.getDoctorId()).username(username).password(encodedPassword).build();
	...
	public Response getAllDoctors() {
		...
		try {
			...
			if (doctors == null || doctors.isEmpty()) {
				response.setSuccess(false);
				response.setData(null);
				response.setMessage("Doctor data not found");
				response.setStatus(HttpStatus.NOT_FOUND.value());
				return response;
			}
			response.setSuccess(true);
...
//-------------------------------------Adding Slots---------------------------------------------------------------------------------------
	...
	public Response saveDoctorSlot(String hospitalId, String doctorId, DoctorSlotDTO dto) {
		...
		try {
			...
			Optional<DoctorSlot> existingSlotOpt = slotRepository.findByDoctorIdAndDate(doctorId, dto.getDate());
			DoctorSlot savedSlot;
			if (existingSlotOpt.isPresent()) {
				DoctorSlot existingSlot = existingSlotOpt.get();
...
//	-------------------------Get Slots by Doctors -------------------------------------------
	...
	public Response getDoctorSlots(String hospitalId, String doctorId) {
		...
		if (slots == null || slots.isEmpty()) {
			throw new ResourceNotFoundException("No slots found for doctor ID: " + doctorId);
		}
...
//--------------------------- detele slot by time and date using doctorId-----------------------------------------
	...
	public Response deleteDoctorSlot(String doctorId, String date, String slotToDelete) {
		Response response = new Response();
		try {
			Optional<DoctorSlot> findDoctorSlot = slotRepository.findByDoctorIdAndDate(doctorId, date);

			if (findDoctorSlot.isEmpty()) {
				response.setSuccess(false);
				response.setData(null);
			...
			DoctorSlot doctorSlot = findDoctorSlot.get();
	...
	public Response deleteDoctorSlotbyDate(String doctorId, String date) {
		Optional<DoctorSlot> slot = slotRepository.findByDoctorIdAndDate(doctorId, date);
		...
		DoctorSlot doctorSlot = slot.get();
	...
	public boolean updateSlot(String doctorId, String date, String time) {
		try {
		Optional<DoctorSlot> doctorSlots = slotRepository.findByDoctorIdAndDate(doctorId, date);
		DoctorSlot d = doctorSlots.get();
		for (DoctorAvailableSlotDTO slot : d.getAvailableSlots()) {
			if (slot.getSlot().equalsIgnoreCase(time)) {
				slot.setSlotbooked(true);
				slotRepository.save(d);
				return true;}}
		return false;
		}catch(Exception e) {
			return false;
...
//-------------------------Get Hopitals and Doctors using SubserviceId------------------------------------------------
	...
	public Response getHospitalAndDoctorsUsingSubserviceId(String subServiceId) {
	    ...
	    try {
	        ...
	        Map<String, List<Doctors>> groupedDoctors = doctors.stream()
	                .collect(Collectors.groupingBy(Doctors::getHospitalId));

	        List<ClinicWithDoctorsDTO> clinics = new ArrayList<>();
	        ObjectMapper mapper = new ObjectMapper();

	        for (Map.Entry<String, List<Doctors>> entry : groupedDoctors.entrySet()) {
	            ...
	            try {
	                ...
	                if (hospitalRes.getStatusCode().is2xxSuccessful() && hospitalRes.getBody() != null) {
	                    Response hospitalResponse = hospitalRes.getBody();

	                    // Use @JsonIgnoreProperties on ClinicDTO to prevent unknown field errors
	                    ClinicDTO clinic = mapper.convertValue(hospitalResponse.getData(), ClinicDTO.class);

	                    ClinicWithDoctorsDTO clinicDTO = new ClinicWithDoctorsDTO();
	                    clinicDTO.setHospitalId(clinic.getHospitalId());
	                    ...
	                    clinicDTO.setHospitalOverallRating(clinic.getHospitalOverallRating());
	                    clinicDTO.setHospitalRegistrations(clinic.getHospitalRegistrations());
	                    clinicDTO.setOpeningTime(clinic.getOpeningTime());
	                    clinicDTO.setClosingTime(clinic.getClosingTime());
	                    clinicDTO.setHospitalLogo(clinic.getHospitalLogo());
	                    clinicDTO.setEmailAddress(clinic.getEmailAddress());
	                    clinicDTO.setWebsite(clinic.getWebsite());
	                    clinicDTO.setLicenseNumber(clinic.getLicenseNumber());
	                    clinicDTO.setIssuingAuthority(clinic.getIssuingAuthority());
	                    clinicDTO.setHospitalDocuments(clinic.getHospitalDocuments());
	                    clinicDTO.setContractorDocuments(clinic.getContractorDocuments());
	                    clinicDTO.setRecommended(clinic.isRecommended());
	                    clinicDTO.setDoctors(doctorList);

	                    clinics.add(clinicDTO);
	                } else {
	                    log.warn("Clinic not found or bad response for hospitalId: {}", hospitalId);
	                }
	            } catch (Exception ex) {
	                log.error("Error fetching clinic for hospitalId {}: {}", hospitalId, ex.getMessage());
	                // Optional: continue fetching other clinics instead of breaking the loop
	            }
	        }
	        ...
	        log.error("Exception occurred in getHospitalAndDoctorsUsingSubserviceId: {}", e.getMessage());
	        response.setSuccess(false);
